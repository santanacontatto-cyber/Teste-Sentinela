...
def score(packet, answer):
    ...
    if packet_errors:
        return {"score":0,"max_score":6,"pass":False,"packet_errors":packet_errors}
    if not isinstance(answer,dict):
        return {"score":0,"max_score":6,"pass":False,"answer_errors":["answer must be object"]}
    unknown=sorted(set(answer)-ALLOWED_ANSWER_FIELDS)
    missing=sorted(ALLOWED_ANSWER_FIELDS-set(answer))
    answer_errors=[]
    ...
    active_ids=exp["active_ids"]
    citations_ok=(isinstance(cited,list) and len(cited)==len(set(cited)) and set(cited)==set(active_ids))
    details["citations"]={"pass":citations_ok,"expected_current_support":active_ids,"got":cited}
    total+=int(citations_ok)

    schema_ok=not answer_errors
    details["answer_schema"]={"pass":schema_ok,"errors":answer_errors}
    total+=int(schema_ok)
