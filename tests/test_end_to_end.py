...
class EndToEndTests(unittest.TestCase):
    def test_reconstruction_demo(self):
        # Run as a module so the repository root stays on sys.path.
        p=subprocess.run([sys.executable,"-m","examples.run_reconstruction_demo"],capture_output=True,text=True)
